...
class VolumeOps(object):
    ...
    def create_volume_for_sm(self, volume, sr_uuid):
        ...
        sm_vol_rec = {}
        try:
            sr_ref = self._session.get_xenapi().SR.get_by_uuid(sr_uuid)
        ...
        vdi_ref = VMHelper.create_vdi(self._session, sr_ref, label, vdi_size,
                                      False)
        vdi_rec = self._session.get_xenapi().VDI.get_record(vdi_ref)
        sm_vol_rec['vdi_uuid'] = vdi_rec['uuid']
        return sm_vol_rec

    def delete_volume_for_sm(self, vdi_uuid):
        vdi_ref = self._session.get_xenapi().VDI.get_by_uuid(vdi_uuid)
        if vdi_ref == None:
            raise exception.Error(_('Could not find VDI ref'))

        try:
            self._session.get_xenapi().VDI.destroy(vdi_ref)
        except self.XenAPI.Failure, exc:
            LOG.exception(exc)
    ...
    def attach_volume(self, instance_name, device_path, mountpoint,
        ...
        sr_params = {}
        if u'sr_uuid' not in dev_params:
            sr_params = VolumeHelper.parse_volume_info(device_path,
                                                        mountpoint,
                                                        dev_params)
            uuid = "FA15E-D15C-" + str(sr_params['id'])
            sr_params['sr_type'] = 'iscsi'
        else:
            uuid = dev_params['sr_uuid']
        ...
        try:
            self._session.get_xenapi().VBD.plug(vbd_ref)
        except self.XenAPI.Failure, exc:
            LOG.exception(exc)
            self.forget_sr(uuid)

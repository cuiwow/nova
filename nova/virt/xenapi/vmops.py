...
class VMOps(object):
    ...
    def _check_host_in_aggregate(self, context, hostname):
        current_aggregate = db.aggregate_get_by_host(context, FLAGS.host)
        try:
            current_aggregate.metadetails[hostname]
        except KeyError:
            reason = _("Destination host:%(hostname) must be in the same "
    ...
    def check_can_live_migrate(self, ctxt, instance_ref, dest,
        ...
        if block_migration:
            ...
            self._check_host_in_aggregate(ctxt, dest)

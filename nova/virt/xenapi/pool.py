...
>>>>>>> 42f6ba6... First attempt at host add
...
=======
            master_hostname = aggregate.metadetails['master_hostname']
            master_compute = aggregate.metadetails['master_compute']
            if master_hostname != self._host_name and master_compute != host:
                # send rpc cast to master, asking to add the following
                # host with specified credentials.
                # NOTE: password in clear is not great, but it'll do for now
                forward_request(context, "add_to_aggregate",
                                master_compute, aggregate, host)
...
=======
    ...
    def remove_from_aggregate(self, context, aggregate, host, **kwargs):
        master_hostname = aggregate.metadetails.get('master_hostname')
        master_compute = aggregate.metadetails.get('master_compute')
        if master_hostname and master_hostname == self._host_name \
           and master_compute and master_compute != host:
            ...
            try:
                ...
                raise exception.AggregateError(aggregate_id=aggregate.id,
                                               action='remove_from_aggregate',
                                               reason=str(e.details))
        elif master_hostname == self._host_name and master_compute == host:
            ...
            try:
                ...
                for key in metadata_keys:
                    ...
                    raise exception.AggregateError(
                                          ...
                                          % locals())
        elif master_hostname and master_hostname != self._host_name \
             and master_compute != host:
            # A master exists -> forward pool-eject request to master
            forward_request(context, "remove_from_aggregate",
                            master_compute, aggregate, host)
        else:
...
def forward_request(context, request_type, recipient, aggregate, sender):
    rpc.cast(context,

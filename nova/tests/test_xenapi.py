...
class XenAPILiveMigrateTestCase(test.TestCase):
    ...
    def tearDown(self):
        super(XenAPILiveMigrateTestCase, self).tearDown()
        self.stubs.UnsetAll()
    ...
    def test_get_host_opaque_ref(self):
        host_ref = xenapi_fake.create_host("test_host")
        host_rec = xenapi_fake.get_record("host", host_ref)

        class FakeAggregate():
            def __init__(self):
                self.metadetails = {"test_host": host_rec["uuid"]}

        def fake_get_aggregate_by_host(context, host):
            self.assertEqual(host, "test_host", "host")
            return FakeAggregate()
        self.stubs.Set(db, "aggregate_get_by_host", fake_get_aggregate_by_host)

        result = self.conn._vmops._get_host_opaque_ref(self.conn, "test_host")

        self.assertEqual(host_ref, result)

    def test_get_host_opaque_ref_on_failure(self):

        class FakeAggregate():
            def __init__(self):
                self.metadetails = {"other_host": "uuid"}
        ...
        self.assertRaises(Exception, self.conn._vmops._get_host_opaque_ref,
                          None, "test_host")
